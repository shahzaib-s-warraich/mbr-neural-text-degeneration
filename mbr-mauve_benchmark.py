...
# ----------------------- Greedy Text Generation MBR (eta (local typical and epsilon), epsilon, top-p, top-k) MAUVE benchmark ---------------------------
...
for name, sampling_config in sampling_configs.items():
    output = generator("Joe Biden", mbr_config = mbr_config, generation_config=sampling_config, tokenizer=tokenizer)[0]['generated_text']
    ...
    print(f"mauve benchmark for {name} sampling:", res)